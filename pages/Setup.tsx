...
import { 
  Monitor, Keyboard, MousePointer, Cpu, Speaker, Coffee, 
  Smartphone, Globe, Code2, Database, Layers, AppWindow, 
  Command, Box, Loader2 
} from 'lucide-react';
...
const MACBOOK_MODEL_URL = 'https://vazxmixjsiawhamofees.supabase.co/storage/v1/object/public/models/macbook/model.gltf';

const MacbookModel = ({ mouseX, mouseY }: { mouseX: number; mouseY: number }) => {
  const { scene } = useGLTF(MACBOOK_MODEL_URL);
  ...
  return (
    <Group ref={group}>
      <Primitive object={scene} position={[0, -1.2, 0]} scale={1.5}>
        <Html 
          transform 
          occlude 
          distanceFactor={1.16} 
          position={[0, 1.05, -1.06]} 
          rotation-x={-0.256}
        >
          <div className="w-[334px] h-[216px] bg-[#050505] overflow-hidden flex flex-col p-4 select-none">
            ...
            <div className="grid grid-cols-4 gap-4 flex-grow content-center">
              {[Code2, Layers, Database, Box, Globe, Cpu, AppWindow, Coffee].map((Icon, idx) => (
                <div key={idx} className="flex flex-col items-center gap-1 animate-pulse" style={{ animationDelay: `${idx * 100}ms` }}>
...
const Setup: React.FC = () => {
  ...
  const gearItems = [
    { icon: Monitor, title: "LG 38\" UltraWide Curved", desc: "Perfect for sprawling Laravel controllers and database schemas." },
    { icon: Globe, title: "IKEA IDÅSEN Desk", desc: "Sturdy, motorized sit-stand desk for long architecture sessions." },
    { icon: Keyboard, title: "Keychron Q1 Max", desc: "Custom mechanical keyboard with brown switches for tactile feedback." },
    { icon: MousePointer, title: "Logitech MX Master 3S", desc: "Precision scrolling through endless logs and documentation." },
      ...
      <div className="max-w-7xl mx-auto space-y-32">
        ...
        <section className="relative h-[85vh] flex flex-col items-center justify-center text-center">
          ...
          <div className="relative z-10 mb-10">
            <span className="text-[#ff6b00] font-mono text-xs uppercase tracking-[0.5em] block mb-4">Workspace Environment</span>
          ...
          <div className="relative w-full h-[500px] md:h-[600px] mt-4 cursor-grab active:cursor-grabbing">
            <Canvas camera={{ position: [0, 0, 5], fov: 45 }} dpr={[1, 2]}>
              <Suspense fallback={<Html center><Loader2 className="w-12 h-12 text-[#ff6b00] animate-spin" /></Html>}>
        ...
        <section className="max-w-4xl mx-auto text-center space-y-12 px-4 reveal-section">
          ...
          <h2 className="text-4xl md:text-6xl font-black tracking-tighter uppercase leading-none">ENGINEERED FOR <span className="text-[#ff6b00] font-serif italic font-light lowercase">precision</span></h2>
          <p className="text-xl md:text-2xl text-slate-400 leading-relaxed font-light">
            My workspace is architected for deep work. Leveraging the efficiency of Apple Silicon M4 with an UltraWide ecosystem allows me to maintain complex domain models in mind while executing performance-critical code.
          </p>
        ...
        <section className="space-y-16">
          <div className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-white/10 pb-8">
            <h2 className="text-4xl md:text-5xl font-black tracking-tighter uppercase">HARDWARE <span className="text-[#ff6b00] font-serif italic font-light lowercase">& gear</span></h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {gearItems.map((item, i) => (
              ...
              >
                ...
                <p className="text-slate-500 text-base leading-relaxed group-hover:text-slate-300 transition-colors">{item.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center pb-24">
          <div className="space-y-12">
            <div className="space-y-4">
              <span className="text-[#ff6b00] font-mono text-xs uppercase tracking-[0.4em] block">Sustained Performance</span>
              <h2 className="text-5xl md:text-7xl font-black tracking-tighter uppercase leading-none">THE CRAFT <br/> <span className="text-[#ff6b00] font-serif italic font-light lowercase">essentials</span></h2>
            </div>
            
            <div className="space-y-8">
              {[
                { title: "Herman Miller Embody", value: "The pinnacle of posture support for marathon architecture reviews." },
                { title: "Elgato Key Light Air", value: "Diffused lighting for professional engineering pair-programming." },
                { title: "Peak Design Tech Ecosystem", value: "Modular storage keeping every bit of hardware precisely organized." }
              ].map((acc, i) => (
                <div key={i} className="flex gap-8 group">
                   ...
                   <div className="space-y-2">
                     <h4 className="text-lg font-bold text-white uppercase tracking-tight group-hover:text-[#ff6b00] transition-colors">{acc.title}</h4>
          ...
          <div className="relative aspect-square md:aspect-[4/5] bg-[#0d0d0d] rounded-[4rem] border border-white/10 overflow-hidden group shadow-2xl">
            ...
            <div className="absolute bottom-12 left-12 right-12 flex justify-between items-end">
              <div>
                <span className="text-xs font-mono text-[#ff6b00] uppercase tracking-widest block mb-3">Reference Image</span>
                <p className="text-xl text-white font-bold uppercase tracking-tighter leading-tight">Minimalist Workspace <br/> Aesthetic</p>
              </div>
