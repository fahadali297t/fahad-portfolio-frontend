      ...
      >
        <div className="relative aspect-[16/10] overflow-hidden">
          ...
          <div className="absolute top-6 left-6 flex items-center gap-2 px-4 py-2 bg-black/60 backdrop-blur-md border border-white/10 rounded-full">
            <div className="w-1.5 h-1.5 bg-[#004aad] rounded-full animate-pulse"></div>
            <span className="text-[9px] font-black uppercase tracking-widest text-slate-300">
              {project.category}
            </span>
          </div>

          <div className="absolute bottom-6 right-6 p-4 bg-[#004aad] rounded-2xl text-black translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 shadow-xl shadow-[#004aad]/20">
        ...
        <div className="p-8 flex flex-col flex-grow space-y-6">
          <div className="space-y-2">
            ...
            <h3 className="text-3xl md:text-4xl font-black tracking-tighter uppercase leading-none group-hover:text-[#004aad] transition-colors">
              {project.title}
              <span className="ml-2 text-xl font-serif italic font-light lowercase text-white">
          ...
          <div className="pt-4 flex flex-wrap gap-3 mt-auto">
            {project.techStackDetailed.slice(0, 3).map((tech, i) => (
              <span
                key={i}
                className="px-3 py-1 bg-white/5 border border-white/5 rounded-lg text-[9px] font-poppins uppercase tracking-widest text-white group-hover:text-white group-hover:border-[#004aad]/20 transition-all"
              >
                {tech}
              </span>
            ))}
...
const Projects: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <section className=" hidden md:flex top-16 z-40 bg-black/90 backdrop-blur-xl border-y border-[#004aad]/10 px-6">
                ...
                className={`px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all duration-300
                  ${
                    filter === cat
                      ...
                      : "bg-white/5 text-white hover:text-white border border-white/5"
                  }`}
              >
                {cat}
              </button>
        ...
        {/* {visibleCount < filteredProjects.length && (
          <div className="mt-32 flex justify-center">
            ...
            >
              ...
              <span className="text-sm font-light text-white uppercase tracking-[0.4em]">
                Load More Data
              </span>
            </button>
          </div>
