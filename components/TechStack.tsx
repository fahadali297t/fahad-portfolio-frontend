...
const TechStack: React.FC = () => {
  ...
  return (
    <section ref={sectionRef} className="py-32 relative bg-slate-50 dark:bg-slate-900/10 overflow-hidden min-h-[700px]">
      ...
      <div className="max-w-7xl mx-auto px-4 w-full relative z-10">
        <div className="text-center mb-24">
          <span className="text-[#ff6b00] font-mono text-sm uppercase tracking-widest font-bold">Stack</span>
          <h2 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white mt-2 mb-4">The Engine Room</h2>
          ...
          {TECH_ICONS.map(({ Icon, label, color }, i) => (
            ...
            >
              <div className="group relative">
                ...
                <div className="absolute -inset-4 bg-gradient-to-tr from-[#ff6b00]/20 to-transparent rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                
                <div className={`relative p-8 rounded-3xl bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 shadow-sm transition-all duration-300 group-hover:border-[#ff6b00] group-hover:shadow-xl group-hover:shadow-orange-500/10 ${color}`}>
                  <Icon size={40} strokeWidth={1.5} />
                </div>
                
                <div className="mt-4 text-center">
                  <span className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 group-hover:text-[#ff6b00] transition-colors">
                    {label}
                  </span>
                </div>
              </div>
